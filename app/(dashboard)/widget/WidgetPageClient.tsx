...
export default function WidgetPageClient({ storeId, agentName }: Props) {
  const [copied, setCopied] = useState(false);

  const embedCode = `<!-- EnigmAI Widget -->
<script>
  window.EnigmAI = {
    storeId: "${storeId}",
    agentName: "${agentName}",
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gradient-to-br from-indigo-100 to-purple-100 rounded-2xl p-8 flex items-end justify-center min-h-[600px] relative">
          ...
          <ChatWidget agentName={agentName} />
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="bg-gray-900 rounded-xl p-4 text-left overflow-auto">
              <pre className="text-green-400 text-xs font-mono whitespace-pre-wrap">{embedCode}</pre>
            </div>
            <button
              onClick={handleCopy}
              className="mt-3 w-full py-2 bg-gray-100 text-gray-700 text-sm rounded-lg hover:bg-gray-200 transition-colors font-medium flex items-center justify-center gap-2"
            >
              {copied ? (
                <>
